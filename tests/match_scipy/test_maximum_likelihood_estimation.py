...
def test_maximum_likelihood_estimation(
    generator: Generator, distribution_info: DistributionInfo[NaturalParametrization]
) -> None:
    ...
    flat_efax_x_clamped = {
        path: flat_parameters[path].domain_support().clamp(value)
        for path, value in flat_efax_x.items()
    }
    efax_x_clamped: Array | dict[str, Any] = (
        flat_efax_x_clamped[()]
